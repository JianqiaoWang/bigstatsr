...
summaries.cv <- function(X, y_diff.train, ind.train, ind.col, ind.sets, K,
  ...
  if(K ==1){
    ...
    SUM_X  <- all_sums$sumX
    SUM_XX <- all_sums$sumXX
    SUM_XY <- all_sums$sumXY
    SUM_Y <- sum(y_diff.train)
    n.sets      <- length(ind.train)
...
#' @keywords internal
#'
COPY_biglasso_main.cv <- function(X, y.train, ind.train, ind.col, covar.train,
...
#' @inheritParams COPY_biglasso_main.cv
...
#'
#' @export
big_spLinReg.cv <- function(X, y.train,
  ...
  do.call(COPY_biglasso_main.cv, args)
}

################################################################################

#' Sparse logistic regression for cross-validation
#'
#' @inheritParams bigstatsr-package
#' @inheritParams COPY_biglasso_main.cv
#' @inheritDotParams COPY_biglasso_main lambda.min.ratio eps max.iter return.all
...
#'
#' @export
big_spLogReg.cv <- function(X, y01.train,
                         ind.train = rows_along(X),
                         ind.col = cols_along(X),
                         covar.train = NULL,
                         base.train = NULL,
